...
export default function DisplayAllGames(props)
...
const handlePicture=(event)=>{
  ...
  setPictureSaveCancel(true)
    ...
    const handleDelete = async () => {
     ...
     var result= await postData("games/deletegames",body)
     if(result)
     {
       swal({
  ...
  var msg="<div>"
    if(isBlank(categoryId))
      ...
      if(isBlank(gameName))
      {error=true
       msg+="<font color='green'><b>Game Name Should not be blank..</b></font><br>"
      }
    ...
    if(isBlank(price))
      {error=true
       msg+="<font color='green'><b>Game Price Should not be blank..</b></font><br>"
      }
  ...
  if (error)
{
  ...
  var body={"gameid":gameId,"subcategoryid":subcategoryId,"categoryid":categoryId,"gamename":gameName,"description":gameDescription,"price":price,"stock":stock,"rented":rented,"rentamt":rentamt,"offer":offer}
  
 
  var result= await postData('games/editgamesdata',body)
  if(result)
  {
    swal({
 ...
 const handleClickSavePicture=async()=>{
  ...
  var result= await postDataAndImage('games/editpicture',formData,config)
  if(result)
  {
    swal({
 ...
 const editFormView=()=>{
  ...
  return (<div className={classes.root}>
   ...
   <Grid item xs={12}>
      ...
      <Grid item xs={12}>
      <TextField value={gameDescription} onChange={(event)=>setGameDescription(event.target.value)} label="Description" variant="outlined" fullWidth/>
      </Grid>
...
const fetchAllGames=async()=>{
  var result=await getData("games/displayallgames")
  setList(result)
  }  
        useEffect(function(){
         fetchAllGames()
